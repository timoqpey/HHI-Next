...
# the global MSVC registry, instantiated by Toolset and BjamToolset (whichever called first)
_msvc_registry = None


class MsvcRegistry(object):

    def __init__(self, os_arch):
        self._supported_msvc_versions = ['14.0', '12.0', '11.0', '10.0', '9.0', '8.0']
    ...
    def is_version_installed(self, version):
        if (ver.version_compare(version, ver.version_tuple_from_str(self._supported_msvc_versions[0])) > 0) or (ver.version_compare(version, ver.version_tuple_from_str(self._supported_msvc_versions[-1])) < 0):
            raise Exception("Microsoft Visual C++ " + ver.version_tuple_to_str(version) + " is not supported.")
        for v in self._installed_msvc_versions:
            if ver.version_compare(v, version) == 0:
                return True
        raise Exception("Microsoft Visual C++ " + ver.version_tuple_to_str(version) + " is not installed.")

    def _do_inventory(self, os_arch):
        ...
        if os_arch == 'x86_64':
            ...
            assert os_arch == 'x86'
            msvc_registry_sections = ['Microsoft']

        reg_key_joiner = "\\"
        self._compiler_command_dict = {}
        # N.B. the express installer seems to create registry keys in the express section and the full product section, hence
        # we need to skip the full product section if an express edition of the same version is installed.
        msvc_express_version_list = []
        for version in self._supported_msvc_versions:
            for section in msvc_registry_sections:
                if version not in msvc_express_version_list:
                    try:
                        sub_key = reg_key_joiner.join(['Software', section, 'VCExpress', version, 'Setup', 'VC' ])
                        ...
                        if os.path.exists(os.path.join(vc_product_dir, 'bin', 'cl.exe')):
                            msvc_express_version_list.append(version)
                        win_registry.CloseKey(rkey)
                    except WindowsError:
                        continue

        # build the list of full product versions given the express list
        real_msvc_versions = []
        for version in self._supported_msvc_versions:
            if version not in msvc_express_version_list:
                real_msvc_versions.append(version)

        msvc_version_list = []
        for version in real_msvc_versions:
            for section in msvc_registry_sections:
                if version not in msvc_version_list:
                    try:
                        ...
                        vc_product_dir = win_registry.QueryValueEx(rkey, 'ProductDir')[0]
                        if os.path.exists(os.path.join(vc_product_dir, 'bin', 'cl.exe')):
                            compiler_cmd = os.path.join(util.normalize_path(vc_product_dir), 'vcvarsall.bat')
                            if os.path.exists(compiler_cmd): 
                                self._compiler_command_dict[version] = compiler_cmd
                                msvc_version_list.append(version)
                        win_registry.CloseKey(rkey)
                    except WindowsError:
                        continue

        # express editions are ignored!!!
        if msvc_version_list:
            self._installed_msvc_versions = ver.version_list_sort([ver.version_tuple_from_str(x) for x in msvc_version_list])
...
class BjamToolset(object):
    def __init__(self, sys_info, bb_version=None):
        ...
        if sys_info.is_linux():
            ...
            # On windows the boost.build version is required as vs 2012 is only supported by bb 1.50.0 or higher.
            assert bb_version is not None

            if _msvc_registry is None:
                _msvc_registry = MsvcRegistry(sys_info.get_os_arch())
            self._msvc_registry = _msvc_registry

            # we have to constrain the search for the latest msvc because the caller must have support for it.
            # e.g. vc11 works only for 1.50.0 or higher
            ...
            # mingw does not work and the visual studio express compilers don't come with 64 bit support.
            self._bjam_toolset = 'msvc-' + ver.version_tuple_to_str(self._msvc_registry.get_latest_version(max_msvc_version))

            # map the msvc toolset spec into a string supported by the bjam build script.
            re_match = re.match(r'^msvc-(\d+)\.\d+.*', self._bjam_toolset)
            assert re_match
            self._bjam_toolset_build_script = 'vc' + re_match.group(1)
...
class Toolset(object):
    ...
    def is_cxx_standard_supported(self, cxx_std):
        if cxx_std in self._cxx_standards:
            return True
        else:
            return False
    ...
    def _qualify_toolset(self, sys_info, toolset, stl='gnustl'):
        ...
        #  2) a relative compiler command: clang++, g++, g++-4.9
        ...
        if self._qualify_android_toolset(sys_info, toolset, stl):
            ...
            if toolset.find('msvc') >= 0:
                ...
                if re_match:
                    ...
                    _msvc_registry.is_version_installed(ver.version_tuple_from_str(re_match.group(1)))
                    ...
                    self._compiler_cmd = _msvc_registry.get_compiler_command(self._version)
                    ...
                    # extract msvc/cl's internal version; e.g. 18.00.40629
                    self._internal_version = self._get_msvc_version(self._compiler_cmd)
                else:
                ...
                if os.path.isabs(toolset):
                    ...
                    if sys_info.is_macosx():
                        ...
                        re_match = re.match(r'(gcc|clang)-([0-9]+\.[0-9]+)$', toolset)
                        if re_match:
                            self._toolset_versioned = toolset
            ...
            if self._toolset == 'msvc':
                ...
                assert False
        # and fill in c++ dialects the compiler supports.
        self._add_cxx_standards()
    ...
    def _add_cxx_standards(self):
        self._cxx_standards = ['c++03'] # all toolsets support c++03
        if self._toolset == 'msvc':
            if ver.version_compare(self._version, (12, 0)) >= 0:
                # used to be enabled for msvc-11.0 and later but msvc-11.0 does not support variadic templates, etc...
                # which boost_fiber requires.
                self._cxx_standards.append('c++11')
        elif self._toolset == 'gcc':
            if ver.version_compare(self._version, (4, 8)) >= 0:
                self._cxx_standards.append('c++11')
            if ver.version_compare(self._version, (4, 9)) >= 0:
                self._cxx_standards.append('c++14')
        elif self._toolset == 'clang':
            # clang comes with different version schemes on macosx and linux. The android clang cross compilers
            # have the same version scheme on macosx and linux.
            target_os = self.get_platform_info(0).get_target_os()
            if target_os == 'macosx':
                if ver.version_compare(self._version, (5, 0)) >= 0:
                    self._cxx_standards.append('c++11')
                if ver.version_compare(self._version, (6, 1)) >= 0:
                    # clang 6.1 is based on llvm 3.6 and released together with xcode 6.3.
                    self._cxx_standards.append('c++14')
            elif target_os in ['linux', 'android']:
                if ver.version_compare(self._version, (3, 3)) >= 0:
                    self._cxx_standards.append('c++11')
                if ver.version_compare(self._version, (3, 5)) >= 0:
                    self._cxx_standards.append('c++14')
            else:
                assert False
        elif self._toolset == 'darwin':
    ...
    def _get_gcc_version(self, gcc_cmd):
        # try gcc -v first to get the full version rather than just major.minor.
        retv = subprocess.check_output([gcc_cmd, '-v'], stderr=subprocess.STDOUT, universal_newlines=True)
        lines = retv.splitlines()
        re_match = re.match(r'gcc\s+version\s+([0-9.]+)', lines[-1])
        if re_match:
            version = ver.version_tuple_from_str(re_match.group(1))
        else:
            # response not understood => gcc -dumpversion
            retv = subprocess.check_output([gcc_cmd, '-dumpversion'], universal_newlines=True)
            version = ver.version_tuple_from_str(retv.rstrip())
        while len(version) < 3:
            #fake a patch level if not reported by gcc -v
    ...
    def _get_msvc_version(self, msvc_cmd):
        msvc_internal_version = None

        tempdir = tempfile.gettempdir()
        script = os.path.join(tempdir, 'pyhhi_msvc_' + ver.version_tuple_to_str(self._version, '_') + '_version.cmd')
        #if os.path.exists(script):
        #    os.remove(script)
        if not os.path.exists(script):
            # create a shell command script to query cl for its internal version.
            with open(script, "w") as script_file:
                script_file.write(textwrap.dedent("""\
                    @ECHO OFF
                """))
                script_file.write('call "' + msvc_cmd + '" amd64 >nul\n')
                script_file.write('cl /?\n')
        # invoke the script through the windows shell.
        retv = subprocess.check_output(script, shell=True, stderr=subprocess.STDOUT, universal_newlines=True).lstrip()
        lines = retv.splitlines()
        assert len(lines) > 0
        re_match = re.match('.*\s+Version\s+([0-9.]+)\s+', lines[0])
        if re_match:
            msvc_internal_version = ver.version_tuple_from_str(re_match.group(1))
        #print("_get_msvc_version: ", lines[0])
        #print("_get_msvc_version: internal version: ", msvc_internal_version)
        return msvc_internal_version
    ...
    def _get_ndk_finder(self):
        if self._ndk_finder is None:
            ...
            self._ndk_finder = android.NdkFinder()
    ...
    def _get_android_api_level(self, toolset_cmd):
        ...
        # the API level can be extracted. If the toolset command refers to the NDK compiler (->QtCreator), the latest API level will
