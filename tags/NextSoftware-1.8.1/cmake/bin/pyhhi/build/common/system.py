...
class SystemInfo(object):
    ...
    class __SystemInfo(object):
        ...
        def __init__(self):
            ...
            self._python3_installed = self._python3_detection()
        ...
        def is_python3_installed(self):
            return self._python3_installed
        ...
        def _query_home_dir(self):
            ...
            if not os.path.exists(home_dir):
                raise Exception('the home directory "' + home_dir + '" does not exist.')
        ...
        def _python3_detection(self):
            python3_installed = False
            if self.is_windows():
                # 1. check for c:\windows\py.exe -> indicates python 3.5 or higher (www.python.org)
                if os.path.exists(os.path.join(r'c:\windows', 'py.exe')):
                    # Helps selecting python3 if python3 and python are installed side-by-side.
                    python3_installed = True
            else:
                fd_null = open('/dev/null')
                # Linux/OSX: /usr/bin/env python3 -V >/dev/null 2>&1 detects python3
                retv = subprocess.call(['/usr/bin/env', 'python3', '-V'], stdout=fd_null, stderr=subprocess.STDOUT)
                if retv == 0:
                    python3_installed = True
            # python3_installed = False
